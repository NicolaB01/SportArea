...
class Ricevuta():
    ...
    def __str__(self):
        return f"Data_emissione:\t{self.data_emissione.strftime('%x')} {self.data_emissione.strftime('%X')}" \
               f"\nPrezzo:\t\t{self.prezzo}" \
               f"\nCliente:\t\t{self.prenotazione.cliente.nome} {self.prenotazione.cliente.cognome}" \
               f"\nCampo:\t\t{self.prenotazione.nome_campo}" \
               f"\nData attività:\t{self.prenotazione.data_attività}"

    def __eq__(self, other):
        return isinstance(other,
                          Ricevuta) and self.data_emissione == other.data_emissione and self.prezzo == other.prezzo and self.prenotazione == other.prenotazione
